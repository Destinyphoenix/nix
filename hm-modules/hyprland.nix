...
# =============================================================================
#  WORKAROUND — home-manager bug #9468
#  https://github.com/nix-community/home-manager/issues/9468
#
#  The new Hyprland *lua* backend mis-emits `$`-prefixed settings keys as
#  `hl.$name(...)`, which is invalid Lua and aborts the whole config with:
#      hyprland.lua:5: <name> expected near '$'   -> emergency mode, no binds.
#
#  Until that bug is fixed we do NOT use Hyprland variables. Instead the literal
#  values are inlined everywhere. Every touched spot is tagged  [HM#9468].
#
#  TO REVERT once #9468 is fixed:
#    1. Uncomment the four variable definitions in the "Variables" block below.
#    2. Undo the inlined literals (search for  [HM#9468]  and the notes):
#         SUPER                          -> $mainMod
#         kitty                          -> $terminal
#         nautilus                       -> $fileManager
#         tofi-drun --drun-launch=true   -> $menu
#    3. gesture: the attrset form is the CORRECT lua syntax and can stay as-is
#       (it is unrelated to #9468 — the old string form was hyprlang-only).
# =============================================================================

{
  ...
  wayland.windowManager.hyprland = {
    enable = true;
    # configType is "lua" by default on stateVersion >= 26.05 — kept implicit.

    settings = {
      # ---- Variables ----
      # [HM#9468] DISABLED — the lua backend can't emit `$`-vars (invalid Lua).
      # Re-enable these (and undo the inlined literals) once #9468 is fixed.
      # "$mainMod" = "SUPER";
      # "$terminal" = "kitty";
      ...
      # lua backend needs the table form (hl.gesture({...})), not a string.
      gesture = {
        fingers = 3;
      ...
      # ---- Keybindings ----
      # [HM#9468] every "SUPER" in this list was "$mainMod".
      #           exec targets that were vars are tagged individually below.
      bind = [
        "SUPER, Q, exec, kitty"                            # [HM#9468] $mainMod, $terminal
        "SUPER, C, killactive"
        "SUPER, M, exit"
        "SUPER, E, exec, nautilus"                         # [HM#9468] $mainMod, $fileManager
        "SUPER, V, togglefloating"
        "SUPER, R, exec, tofi-drun --drun-launch=true"     # [HM#9468] $mainMod, $menu
        "SUPER, P, pseudo"
        "SUPER, B, exec, brave --password-store=basic"
        ...
        "SUPER, F9,  exec, hyprctl hyprsunset identity"
        "SUPER, F10, exec, hyprctl hyprsunset temperature 4000"
      ];

      # [HM#9468] "SUPER" below was "$mainMod".
      bindel = [
        ", XF86MonBrightnessUp, exec, brightnessctl s +5%"
        ", XF86MonBrightnessDown, exec, brightnessctl s 5%-"
        "SUPER ALT, up, exec, ddcutil setvcp 10 +10"
        "SUPER ALT, down, exec, ddcutil setvcp 10 -10"
